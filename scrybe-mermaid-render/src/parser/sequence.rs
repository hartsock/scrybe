...
pub fn parse(source: &str) -> Result<SequenceDiagram> {
    ...
    for raw_line in source.lines() {
        ...
        if let Some((from, to)) = parse_arrow_endpoints(line) {
            register_participant(&mut diagram.participants, from);
            register_participant(&mut diagram.participants, to);
        }
    }
...
fn parse_participant_declaration(line: &str) -> Result<Option<Participant>> {
    let Some((keyword, rest)) = split_first_word(line) else {
        return Ok(None);
    };
    ...
    let (alias, display) = match split_as_clause(rest) {
        ...
        Some(_) => {
            return Err(MermaidRenderError::Parse(format!(
                "invalid participant declaration `{line}`",
            )))
        }
...
fn split_as_clause(rest: &str) -> Option<(&str, &str)> {
    ...
    let (before, _) = lower.split_once(" as ")?;
    let split_at = before.len();

    Some((rest[..split_at].trim(), rest[split_at + " as ".len()..].trim()))
}
...
fn register_participant(participants: &mut Vec<Participant>, alias: &str) {
...
mod tests {
    ...
    #[test]
    fn test_parse_later_declaration_updates_auto_registered_participant() {
        let src = "sequenceDiagram\n  Alice->>Bob: Hello\n  actor Bob as B";
        let diagram = parse(src).unwrap();
        assert_participants(
            &diagram,
