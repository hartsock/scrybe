...
All dependencies in the default build are MIT or MIT/Apache-2.0 dual-licensed.
No licensing obligations beyond attribution.

### With `--features png`

The `png` feature adds `resvg` and `usvg`, both licensed **MPL-2.0**.

| Crate | Version | License |
|---|---|---|
| `resvg` | 0.44.x | MPL-2.0 |
| `usvg` | 0.44.x | MPL-2.0 |
| `tiny-skia` | 0.11.x | BSD-3-Clause |
| `fontdb` | 0.22.x | MIT |
| `roxmltree` | 0.20.x | MIT OR Apache-2.0 |
| `ttf-parser` | 0.24.x | MIT OR Apache-2.0 |

**MPL-2.0 is compatible with Apache-2.0.** MPL-2.0 §3.3 (Larger Works)
explicitly permits combining MPL-2.0 libraries with code under other
licenses, including Apache-2.0. The FSF and OSI confirm this compatibility.

Obligations when distributing a build with `--features png`:
- Include the MPL-2.0 license text for `resvg` and `usvg`
- Make `resvg` and `usvg` source available (crates.io satisfies this automatically)
- Any modifications made to `resvg` or `usvg` source files must be released
  under MPL-2.0 — but using them unmodified as dependencies carries no such
  requirement

Since `publish = false` and `png` is opt-in, no immediate action is needed.
Before promoting to `publish = true`, add a `LICENSE-MPL-2.0` file and note
the optional MPL-2.0 dependency in the crate-level documentation.

### Workspace MPL-2.0 (not from this crate)

`cssparser`, `selectors`, `dtoa-short`, and `option-ext` (all MPL-2.0) are
present in the workspace dependency tree via Tauri (`scrybe-app/src-tauri`).
They have no bearing on `scrybe-mermaid-render`, which does not depend on
Tauri. Confirmed with `cargo metadata` dep-tree analysis.

### Test fixtures and oracle

| Source | License | Usage |
|---|---|---|
| `tests/fixtures/` (hand-written) | Apache-2.0 (ours) | Committed to repo |
| `mermaid-cli test-positive/` | MIT | Fetched at test time by `gen_oracle.sh`, not committed |
| Oracle SVGs (generated by mmdc) | — | Derived output, not committed, gitignored |

The upstream mermaid-cli fixtures are MIT licensed and fetched at runtime
(`gen_oracle.sh --with-upstream`). They are never committed to this repo
so no license file inclusion is required — but attribution is good practice.
The `--with-upstream` flag comment in `gen_oracle.sh` notes the MIT license
and source URL.
